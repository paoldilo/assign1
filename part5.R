...
TDM_5words <- TermDocumentMatrix(mainCorpus, control = list(tokenize = FivegramTokenizer))

dictMatrix <- as.data.frame(cbind(TDM_5words$dimnames$Terms,as.numeric(TDM_5words$v)),stringsAsFactors=FALSE)
...
dictMatrix2 <- as.data.frame(cbind(TDM_5words$dimnames$Terms,as.numeric(TDM_5words$v)),stringsAsFactors=FALSE)
...
###final assembly
##################
write.table(dictMatrix,file="dictionary_5w_full_1.txt",quote=F,sep=";")
...
write.table(dictMatrix,file="dictionary_5w_1.txt",quote=F,sep=";")
gc()